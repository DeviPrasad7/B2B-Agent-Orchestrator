"""Add status to processed_events

Revision ID: cf8920b8b7e3
Revises: 
Create Date: 2026-06-27 14:02:41.597371
...
def upgrade() -> None:
    ...
    op.alter_column('hitl_requests', 'created_at',
               existing_type=postgresql.TIMESTAMP(),
               type_=sa.DateTime(timezone=True),
               existing_nullable=True)
    op.alter_column('hitl_requests', 'resolved_at',
               existing_type=postgresql.TIMESTAMP(),
               type_=sa.DateTime(timezone=True),
               existing_nullable=True)
    op.create_index(op.f('ix_hitl_requests_decision'), 'hitl_requests', ['decision'], unique=False)
    op.add_column('processed_events', sa.Column('status', sa.String(), nullable=False))
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_hitl_requests_decision'), table_name='hitl_requests')
    op.alter_column('hitl_requests', 'resolved_at',
               existing_type=sa.DateTime(timezone=True),
               type_=postgresql.TIMESTAMP(),
               existing_nullable=True)
